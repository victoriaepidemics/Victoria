...
from victoriaepi.ama_grp import ama2_grp
...
exit_probs =[array([0.05]*ngrp), F_g,  F_h, array([0.5]*ngrp)]
...
Int_M = ones((ngrp,ngrp))/ngrp #Uniform
...
def AnalyzeZM( clave, T, burnin=5000, init=date(2020, 4, 1), trim=0, pred=100, plot_fit=True, workdir="./"):
    ...
    init=init
    if (ZMs[clave][3]+ timedelta(days=4)) <= init:
        # Incluida el corrimiento de 4 dias (inicio de sintomas)
        init_index=(init - (ZMs[clave][3] + timedelta(days=4))).days
    else: # Incluida el corrimiento de 4 dias (inicio de sintomas)
        init=ZMs[clave][3] + timedelta(days=4)
        init_index=0
...
if __name__=='__main__':
    ...
    if len(sys.argv) > 1:
        ...
        T=10000 # 20000, Usual number of iterations used for MCMC
        # Analyze a list of metro zones
        clave_list=['9-01']#, '15-02', '31-01', '17-02', '12-01', '25-01', '23-01']

        for clave in clave_list:

            trim=0
            init=date(2020, 4, 1)
            pred=7*4
            print( "\n%s, %s, T=%d, trim=%d" % ( clave, ZMs[clave][0], T, trim))
            ZMs[clave][-1] = AnalyzeZM(clave, T=T, trim=trim, init=init, pred=200, plot_fit=False, workdir=workdir)
            ZMs[clave][-1].clave = clave
            zm = ZMs[clave][-1]
            # Custom plots for a metro zone
            PlotFigsZMs(ZMs[clave][-1], pred=pred, blue=True, workdir=workdir)
